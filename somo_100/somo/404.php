        ...
        <?php if(!empty(yprm_get_theme_setting('404_heading')) || !empty(yprm_get_theme_setting('404_sub_heading'))) { ?>
            <div class="heading-block">
              ...
              <?php } if($heading = yprm_get_theme_setting('404_heading')) { ?>
                <div class="h h2"><?php echo wp_kses($heading, 'post') ?></div>
              <?php } ?>
            </div>
          <?php } if($text = yprm_get_theme_setting('404_text')) { ?>
          <div class="text"><?php echo wp_kses($text, 'post') ?></div>
          <?php } ?>
          <a class="button-style1 accent" href="<?php echo esc_url(home_url('/')) ?>" target="_self" data-magic-cursor="link"><i class="pointers-left-arrow"></i><span><?php echo esc_html__('Get Back Home', 'somo') ?></span></a>
        </div>
      </div>
    </div>
  </div>
