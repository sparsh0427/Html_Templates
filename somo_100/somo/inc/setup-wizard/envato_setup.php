  ...
  class Envato_Theme_Setup_Wizard {
    ...
    public function init_actions() {
      if (apply_filters($this->theme_name . '_enable_setup_wizard', true) && current_user_can('manage_options')) {
        ...
        if (class_exists('TGM_Plugin_Activation') && isset($GLOBALS['somo'])) {
    ...
    public function get_tgmpa_instanse() {
      $this->tgmpa_instance = call_user_func(array(get_class($GLOBALS['somo']), 'get_instance'));
    }
    ...
    public function admin_menus() {
      ...
      $add_subpage_function('somo_dashboard', esc_html__('Setup Wizard', 'somo'), esc_html__('Setup Wizard', 'somo'), 'manage_options', $this->page_slug, array(
        $this,
        'setup_wizard',
      ));
    }
    ...
    public function init_wizard_steps() {
      ...
      $this->steps = array(
        'introduction' => array(
          'name' => esc_html__('Introduction', 'somo'),
    ...
    public function setup_wizard() {
      ...
      wp_localize_script('envato-setup', 'envato_setup_params', array(
        ...
        'verify_text' => esc_html__('...verifying', 'somo'),
      ));

      wp_enqueue_style('somo-envato-setup', get_parent_theme_file_uri() . '/css/admin.css', array(
        'wp-admin',
        'dashicons',
        'install',
      ), $this->version);
    ...
    public function setup_wizard_header() {
      ?>
		...
		<head>
			...
			<?php
      echo '<title>' . esc_html__('Theme &rsaquo; Setup Wizard', 'somo') . '</title>'; ?>
      ...
      if ($image_url) {
        $image = '<img class="site-logo" src="%s" alt="%s" style="height: 18px;"/>';
        printf(
          $image,
          $image_url,
          get_bloginfo('name'),
      ...
      ?>
		<?php if ('next_steps' === $this->step): ?>
			<a class="wc-return-to-dashboard"
			   href="<?php echo esc_url(admin_url()); ?>"><?php esc_html_e('Return to the WordPress Dashboard', 'somo'); ?></a>
		<?php endif; ?>
		</body>
		<?php
    ...
    public function envato_setup_introduction() {
      ...
      if (false && isset($_REQUEST['debug'])) {
        ...
        ?>
				<h1><?php printf(esc_html__('Welcome to the setup wizard for %s.', 'somo'), wp_get_theme()); ?></h1>
				<p><?php esc_html_e('It looks like you may have recently upgraded to this theme. Great! This setup wizard will help ensure all the default settings are correct. It will also show some information about your new website and support options.', 'somo'); ?></p>
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url($this->get_next_step_link()); ?>"
					   class="button-primary button button-large button-next"><?php esc_html_e('Let\'s Go!', 'somo'); ?></a>
					<a href="<?php echo esc_url(wp_get_referer() && !strpos(wp_get_referer(), 'update.php') ? wp_get_referer() : admin_url('')); ?>"
					   class="button button-large"><?php esc_html_e('Not right now', 'somo'); ?></a>
				</p>
				<?php
} elseif (get_option('envato_setup_complete', false)) {
        ?>
				<h1><?php printf(esc_html__('Welcome to the setup wizard for %s.', 'somo'), wp_get_theme()); ?></h1>
				<p><?php esc_html_e('It looks like you have already run the setup wizard. Below are some options: ', 'somo'); ?></p>
				<ul>
					<li>
						<a href="<?php echo esc_url($this->get_next_step_link()); ?>"
						   class="button-primary button button-next button-large"><?php esc_html_e('Run Setup Wizard Again', 'somo'); ?></a>
					</li>
					<li>
						<form method="post">
							<input type="hidden" name="reset-font-defaults" value="yes">
							<input type="submit" class="button-primary button button-large button-next"
							       value="<?php esc_attr_e('Reset font style and colors', 'somo'); ?>" name="save_step"/>
				...
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url(wp_get_referer() && !strpos(wp_get_referer(), 'update.php') ? wp_get_referer() : admin_url('')); ?>"
					   class="button button-large"><?php esc_html_e('Cancel', 'somo'); ?></a>
				</p>
				<?php
} else {
        ?>
				<h1><?php printf(esc_html__('Welcome to the setup wizard for %s.', 'somo'), wp_get_theme()); ?></h1>
				<p><?php printf(esc_html__('Thank you for choosing the %s Wordpress Theme. This quick setup wizard will help you configure your new website. This wizard will install the required WordPress plugins, default content, logo and tell you a little about Help &amp; Support options. It should only take 5 minutes.', 'somo'), wp_get_theme()); ?></p>
				<p><?php esc_html_e('No time right now? If you don\'t want to go through the wizard, you can skip and return to the WordPress dashboard. Come back anytime if you change your mind!', 'somo'); ?></p>
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url($this->get_next_step_link()); ?>"
					   class="button-primary button button-large button-next"><?php esc_html_e('Let\'s Go!', 'somo'); ?></a>
					<a href="<?php echo esc_url(wp_get_referer() && !strpos(wp_get_referer(), 'update.php') ? wp_get_referer() : admin_url('')); ?>"
    ...
    public function envato_setup_introduction_save() {
      ...
      if (!empty($_POST['reset-font-defaults']) && $_POST['reset-font-defaults'] == 'yes') {
        ...
        ?>
				<p>
					<strong><?php esc_html_e('Options have been reset. Please go to Appearance > Customize in the WordPress backend.', 'somo'); ?></strong>
				</p>
      <?php
        return true;
      }
    ...
    private function __get_plugins() {
      $instance = call_user_func(array(get_class($GLOBALS['somo']), 'get_instance'));
      $plugins = array(
    ...
    public function envato_setup_default_plugins() {
      ...
      ?>
			<h1><?php esc_html_e('Default Plugins', 'somo'); ?></h1>
			<form method="post">
      ...
      if (count($plugins['all'])) {
        ?>
					<p><?php esc_html_e('Your website needs a few essential plugins. The following plugins will be installed or updated:', 'somo'); ?></p>
        ...
        echo '<p><strong>' . esc_html_e('Good news! All plugins are already installed and up to date. Please continue.', 'somo') . '</strong></p>';
      } ?>

				<p><?php esc_html_e('You can add and remove plugins later on from within WordPress.', 'somo'); ?></p>

				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url($this->get_next_step_link()); ?>" class="button-primary button button-large button-next" data-callback="install_plugins"><?php esc_html_e('Continue', 'somo'); ?></a>
					<a href="<?php echo esc_url($this->get_next_step_link()); ?>" class="button button-large button-next"><?php esc_html_e('Skip this step', 'somo'); ?></a>
					<?php wp_nonce_field('envato-setup'); ?>
				</p>
			</form>
			<?php
    ...
    public function ajax_plugins() {
      if (!check_ajax_referer('envato_setup_nonce', 'wpnonce') || empty($_POST['slug'])) {
        wp_send_json_error(array('error' => 1, 'message' => esc_html__('No Slug Found', 'somo')));
      }
      $json = array();
      ...
      foreach ($plugins['activate'] as $slug => $plugin) {
        if ($_POST['slug'] == $slug) {
          $json = array(
            ...
            'action2' => -1,
            'message' => esc_html__('Activating Plugin', 'somo'),
          );
          break;
        }
      }
      foreach ($plugins['update'] as $slug => $plugin) {
        if ($_POST['slug'] == $slug) {
          $json = array(
            ...
            'action2' => -1,
            'message' => esc_html__('Updating Plugin', 'somo'),
          );
          break;
        }
      }
      foreach ($plugins['install'] as $slug => $plugin) {
        if ($_POST['slug'] == $slug) {
          $json = array(
            ...
            'action2' => -1,
            'message' => esc_html__('Installing Plugin', 'somo'),
          );
          break;
        }
      }
      ...
      if ($json) {
        ...
        wp_send_json(array('done' => 1, 'message' => esc_html__('Success', 'somo')));
      }
      exit;
    ...
    private function __content_default_get() {
      ...
      foreach ($available_content as $post_type => $post_data) {
        if (count($post_data)) {
          ...
          $content[$post_type] = array(
            ...
            'description' => sprintf(esc_html__('This will create default %s as seen in the demo.', 'somo'), $post_type_title),
            'pending' => esc_html__('Pending.', 'somo'),
            'installing' => esc_html__('Installing.', 'somo'),
            'success' => esc_html__('Success.', 'somo'),
      ...
      $content['widgets'] = array(
        'title' => esc_html__('Widgets', 'somo'),
        'description' => esc_html__('Insert default sidebar widgets as seen in the demo.', 'somo'),
        'pending' => esc_html__('Pending.', 'somo'),
        'installing' => esc_html__('Installing Default Widgets.', 'somo'),
        'success' => esc_html__('Success.', 'somo'),
      ...
      $content['settings'] = array(
        'title' => esc_html__('Settings', 'somo'),
        'description' => esc_html__('Configure default settings.', 'somo'),
        'pending' => esc_html__('Pending.', 'somo'),
        'installing' => esc_html__('Installing Default Settings.', 'somo'),
        'success' => esc_html__('Success.', 'somo'),
    ...
    public function envato_setup_default_content() {
      ?>
			<h1><?php esc_html_e('Default Content', 'somo'); ?></h1>
			<form method="post">
				<?php if ($this->is_possible_upgrade()) { ?>
					<p><?php esc_html_e('It looks like you already have content installed on this website. If you would like to install the default demo content as well you can select it below. Otherwise just choose the upgrade option to ensure everything is up to date.', 'somo'); ?></p>
				<?php } else { ?>
					<p><?php printf(esc_html__('It\'s time to insert some default content for your new WordPress website. Choose what you would like inserted below and click Continue. It is recommended to leave everything selected. Once inserted, this content can be managed from the WordPress admin dashboard. ', 'somo'), '<a href="' . esc_url(admin_url('edit.php?post_type=page')) . '" target="_blank">', '</a>'); ?></p>
				<?php } ?>
				<table class="envato-setup-pages" cellspacing="0">
					<thead>
					<tr>
						<td class="check"></td>
						<th class="item"><?php esc_html_e('Item', 'somo'); ?></th>
						<th class="description"><?php esc_html_e('Description', 'somo'); ?></th>
						<th class="status"><?php esc_html_e('Status', 'somo'); ?></th>
					</tr>
					</thead>
					<tbody>
				...
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url($this->get_next_step_link()); ?>"
					   ...
					   data-callback="install_content"><?php esc_html_e('Continue', 'somo'); ?></a>
					<a href="<?php echo esc_url($this->get_next_step_link()); ?>"
					   class="button button-large button-next"><?php esc_html_e('Skip this step', 'somo'); ?></a>
					<?php wp_nonce_field('envato-setup'); ?>
				</p>
			</form>
			<?php
    ...
    public function ajax_content() {
      ...
      if (!check_ajax_referer('envato_setup_nonce', 'wpnonce') || empty($_POST['content']) && isset($content[$_POST['content']])) {
        wp_send_json_error(array('error' => 1, 'message' => esc_html__('No content Found', 'somo')));
      }
    ...
    private function __fetch_remote_file($url, $post) {
      ...
      if (!$upload || $upload['error']) {
        ...
        if (is_array($response) && !empty($response['body']) && $response['response']['code'] == '200') {
          ...
          return new WP_Error('import_file_error', esc_html__('Remote server did not respond', 'somo'));
        }
        ...
        if (isset($headers['content-length']) && $filesize != $headers['content-length']) {
          ...
          return new WP_Error('import_file_error', esc_html__('Remote file is incorrect size', 'somo'));
        }

        if (0 == $filesize) {
          @unlink($upload['file']);

          return new WP_Error('import_file_error', esc_html__('Zero size file downloaded', 'somo'));
        }

        if (!empty($max_size) && $filesize > $max_size) {
          @unlink($upload['file']);

          return new WP_Error('import_file_error', sprintf(esc_html__('Remote file is too large, limit is %s', 'somo'), size_format($max_size)));
        }
      }
    ...
    public function envato_setup_color_style() {
      ...
      ?>
            <h1><?php esc_html_e('Site Style', 'somo'); ?></h1>
            <form method="post">
                <p><?php esc_html_e('Please choose your site style below.', 'somo'); ?></p>
                ...
                <p><em><?php echo esc_html__('Please Note: Advanced changes to website graphics/colors may require extensive PhotoShop and Web Development knowledge. We recommend hiring an expert from', 'somo') ?> <a href="http://studiotracking.envato.com/aff_c?offer_id=4&aff_id=1564&source=DemoInstall" target="_blank"><?php echo esc_html__('Envato Studio', 'somo') ?></a> <?php echo esc_html__('to assist with any advanced website changes.', 'somo') ?></em></p>
                <div style="display: none;">
                    <img src="http://studiotracking.envato.com/aff_i?offer_id=4&aff_id=1564&source=DemoInstall" width="1" height="1"/>
                </div>

                <p class="envato-setup-actions step">
                  <input type="submit" class="button-primary button button-large button-next" value="<?php esc_attr_e('Continue', 'somo'); ?>" name="save_step"/>
                  <a href="<?php echo esc_url($this->get_next_step_link()); ?>" class="button button-large button-next"><?php esc_html_e('Skip this step', 'somo'); ?></a>
                  <?php wp_nonce_field('envato-setup'); ?>
                </p>
            </form>
    ...
    public function envato_setup_logo_design() {
      ...
      ?>
			<h1><?php esc_html_e('Logo', 'somo'); ?></h1>
			<form method="post">
				<p><?php printf(esc_html__('Please add your logo below. For best results, the logo should be a transparent PNG ( 466 by 277 pixels). The logo can be changed at any time from the Appearance > Customize area in your dashboard. Try %sEnvato Studio%s if you need a new logo designed.', 'somo'), '<a href="http://studiotracking.envato.com/aff_c?offer_id=4&aff_id=1564&source=DemoInstall" target="_blank">', '</a>'); ?></p>
      ...
      if ($image_url) {
        ...
        printf(
          ...
          $this->get_header_logo_width()
						...
						<td>
							<a href="#" class="button button-upload"><?php esc_html_e('Upload New Logo', 'somo'); ?></a>
						</td>
					</tr>
				</table>

				<p><em><?php echo esc_html__('Please Note: Advanced changes to website graphics/colors may require extensive PhotoShop and Web
						Development knowledge. We recommend hiring an expert from', 'somo') ?> <a
							href="http://studiotracking.envato.com/aff_c?offer_id=4&aff_id=1564&source=DemoInstall"
							target="_blank"><?php echo esc_html__('Envato Studio', 'somo') ?></a> <?php echo esc_html__('to assist with any advanced website changes.', 'somo') ?></em></p>
				<div style="display: none;">
					<img src="http://studiotracking.envato.com/aff_i?offer_id=4&aff_id=1564&source=DemoInstall" width="1" height="1"/>
				</div>
				...
				<p class="envato-setup-actions step">
					<input type="submit" class="button-primary button button-large button-next"
					       value="<?php esc_attr_e('Continue', 'somo'); ?>" name="save_step"/>
					<a href="<?php echo esc_url($this->get_next_step_link()); ?>"
					   class="button button-large button-next"><?php esc_html_e('Skip this step', 'somo'); ?></a>
					<?php wp_nonce_field('envato-setup'); ?>
				</p>
    ...
    public function envato_setup_updates() {
      ?>
			<h1><?php esc_html_e('Theme Updates', 'somo'); ?></h1>
...
$option = envato_market()->get_options();
        ...
        if (count($my_items)) {
          ?>
						<p><?php echo esc_html__('Thanks! Theme updates have been enabled for the following items:', 'somo') ?></p>
						<ul>
							<?php foreach ($my_items as $item) { ?>
								<li><?php echo esc_html($item['name']); ?></li>
							<?php } ?>
						</ul>
						<p><?php echo esc_html__('When an update becomes available it will show in the Dashboard with an option to install.', 'somo') ?></p>
						<p><?php echo esc_html__('Change settings from the \'Envato Market\' menu in the WordPress Dashboard.', 'somo') ?></p>

						<p class="envato-setup-actions step">
							<a href="<?php echo esc_url($this->get_next_step_link()); ?>"
							   class="button button-large button-next button-primary"><?php esc_html_e('Continue', 'somo'); ?></a>
						</p>
        <?php
          } else {
        ?>
						<p><?php esc_html_e('Please login using your ThemeForest account to enable Theme Updates. We update themes when a new feature is added or a bug is fixed. It is highly recommended to enable Theme Updates.', 'somo'); ?></p>
						<p><?php echo esc_html__('When an update becomes available it will show in the Dashboard with an option to install.', 'somo') ?></p>
						<p>
							<em><?php echo esc_html__('On the next page you will be asked to Login with your ThemeForest account and grant permissions to enable Automatic Updates. If you have any questions please', 'somo') ?> <a href="http://yprm.net/envato/" target="_blank"><?php echo esc_html__('contact us', 'somo') ?></a>.</em>
						</p>
						<p class="envato-setup-actions step">
							<input type="submit" class="button-primary button button-large button-next" value="<?php esc_attr_e('Login with Envato', 'somo'); ?>" name="save_step"/>
							<a href="<?php echo esc_url($this->get_next_step_link()); ?>" class="button button-large button-next"><?php esc_html_e('Skip this step', 'somo'); ?></a>
							<?php wp_nonce_field('envato-setup'); ?>
						</p>
					<?php } ?>
			...
			<?php } else { ?>
				<?php echo esc_html__('Please ensure the Envato Market plugin has been installed correctly.', 'somo') ?> <a href="<?php echo esc_url($this->get_step_link('default_plugins')); ?>"><?php echo esc_html__('Return to Required Plugins installer', 'somo') ?></a>.
			<?php } ?>
			<?php
}
    ...
    public function envato_setup_customize() {
      ?>
			...
			<h1><?php echo esc_html__('Theme Customization', 'somo') ?></h1>
			<p><?php echo esc_html__('Most changes to the website can be made through the Appearance > Customize menu from the WordPress dashboard. These include:', 'somo') ?></p>
			<ul>
				<li><?php echo esc_html__('Typography: Font Sizes, Style, Colors (over 200 fonts to choose from) for various page elements.', 'somo') ?></li>
				<li><?php echo esc_html__('Logo: Upload a new logo and adjust its size.', 'somo') ?></li>
				<li><?php echo esc_html__('Background: Upload a new background image.', 'somo') ?></li>
				<li><?php echo esc_html__('Layout: Enable/Disable responsive layout, page and sidebar width.', 'somo') ?></li>
			</ul>
			<p><?php echo wp_kses_post(__('To change the Sidebars go to Appearance > Widgets. Here widgets can be "drag &amp; droped" into sidebars.<br>To control which "widget areas" appear, go to an individual page and look for the "Left/Right Column" menu. Here widgets can be chosen for display on the left or right of a page. More details in documentation.', 'somo')) ?></p>
			<p>
				<em><?php echo wp_kses_post(__('Advanced Users: If you are going to make changes to the theme source code please use a <a href="https://codex.wordpress.org/Child_Themes" target="_blank">Child Theme</a> rather than modifying the main theme HTML/CSS/PHP code. This allows the parent theme to receive updates without overwriting your source code changes. <br/> See <code>child-theme.zip</code> in the main folder for a sample.', 'somo')) ?></em>
			</p>

			<p class="envato-setup-actions step">
				<a href="<?php echo esc_url($this->get_next_step_link()); ?>"
				   class="button button-primary button-large button-next"><?php esc_html_e('Continue', 'somo'); ?></a>
			</p>
    ...
    public function envato_setup_help_support() {
      ?>
        <h1><?php echo esc_html__('Help and Support', 'somo') ?></h1>
        <p><?php echo wp_kses_post(__('This theme includes 6 months of support (from the purchase date). The standard license allows using one license on one Project (domain). If you need to use it on different sites - you need to purchase additional licenses. ThemeForest <a href="https://themeforest.net/licenses/standard" target="_blank">license policy</a>', 'somo')) ?></p>
        <p><?php echo wp_kses_post(__('If you have some problems, please contact our support team <a href="https://themeforest.net/item/somo-creative-multipurpose-wordpress-theme/25125484/support" target="_blank">here</a>', 'somo')) ?></p>
          
        <div class="yprm-support-block">
          <div class="title"><?php echo wp_kses_post(__('Item Support <span class="green">INCLUDES</span>:', 'somo')) ?></div>
          <ul>
            <li><?php echo esc_html__('- Availability of the author to answer questions', 'somo') ?></li>
            <li><?php echo esc_html__('- Answering technical questions about item features', 'somo') ?></li>
            <li><?php echo esc_html__('- Assistance with reported bugs and issues', 'somo') ?></li>
            <li><?php echo esc_html__('- Help with included 3rd party plugins', 'somo') ?></li>
          </ul>
        </div>
        
        <div class="yprm-support-block">
          <div class="title"><?php echo wp_kses_post(__('Item Support <span class="red">DOES NOT</span> Include:', 'somo')) ?></div>
          <ul>
            <li><?php echo esc_html__('- Customization services (this is available by an additional price)', 'somo') ?></li>
            <li><?php echo esc_html__('- Installation services (this is available an additional price)', 'somo') ?></li>
            <li><?php echo esc_html__('- Help and Support for non-bundled 3rd party plugins (i.e. plugins you install yourself later on)', 'somo') ?></li>
          </ul>
        </div>

        <p><?php echo wp_kses_post(__('ThemeForest item <a href="https://help.market.envato.com/hc/en-us/articles/208191263-What-is-Item-Support" target="_blank">support policy</a>', 'somo')) ?></p>

        <p class="envato-setup-actions step">
          <a href="<?php echo esc_url($this->get_next_step_link()); ?>" class="button button-primary button-large button-next"><?php esc_html_e('Agree and Continue', 'somo'); ?></a>
          <?php wp_nonce_field('envato-setup'); ?>
        </p>
			<?php
}
    ...
    ?>
			...
			<h1><?php esc_html_e('Your Website is Ready!', 'somo'); ?></h1>

			<p><?php echo esc_html__('Congratulations! The theme has been activated and ready to work. Login to your WordPress Dashboard to make changes and modify necessary page.', 'somo') ?></p>
			<p><?php echo wp_kses_post(__('Please leave a 5-star rating if you are happy with our theme.', 'somo')) ?></p>
			<div class="envato-setup-next-steps">
				<div class="envato-setup-next-steps-first">
					<h2><?php esc_html_e('Next Steps', 'somo'); ?></h2>
					<ul>
						<li class="setup-product"><a class="button button-next button-large" href="<?php echo esc_url(home_url()); ?>"><?php esc_html_e('View your new website!', 'somo'); ?></a>
						</li>
					</ul>
				</div>
				<div class="envato-setup-next-steps-last">
					<h2><?php esc_html_e('More Resources', 'somo'); ?></h2>
					<ul>
						<li class="documentation"><a href="https://promo-theme.com/somo/docs/" target="_blank"><?php esc_html_e('Read the Theme Documentation', 'somo'); ?></a>
						</li>
						<li class="howto"><a href="https://wordpress.org/support/" target="_blank"><?php esc_html_e('Learn how to use WordPress', 'somo'); ?></a>
						</li>
						<li class="rating"><a href="https://themeforest.net/item/somo-creative-multipurpose-wordpress-theme/25125484" target="_blank"><?php esc_html_e('Leave an Item Rating', 'somo'); ?></a></li>
						<li class="support"><a href="https://themeforest.net/item/somo-creative-multipurpose-wordpress-theme/25125484/support" target="_blank"><?php esc_html_e('Get Help and Support', 'somo'); ?></a></li>
					</ul>
				</div>
			</div>
    <?php }
    ...
    public function envato_market_admin_init() {
      ...
      if (!empty($_POST['oauth_session']) && !empty($_POST['bounce_nonce']) && wp_verify_nonce($_POST['bounce_nonce'], 'envato_oauth_bounce_' . $this->envato_username)) {
        ...
        if (is_wp_error($response)) {
          ...
          $class = 'error';
          echo "<div class=\"$class\"><p>" . sprintf(esc_html__('Something went wrong while trying to retrieve oauth token: %s', 'somo'), $error_message) . '</p></div>';
        } else {
          ...
          if ($result !== true) {
            echo esc_html__('Failed to get oAuth token. Please go back and try again', 'somo');
            exit;
          }
        }
      }
      ...
      add_settings_section(
        ...
        sprintf(esc_html__('Login for %s updates', 'somo'), $this->envato_username),
    ...
    public function render_oauth_login_description_callback() {
      //echo esc_html__('If you have purchased items from ' . $this->envato_username . ' on ThemeForest or CodeCanyon please login here for quick and easy updates.', 'somo');
      ...
      ?>
			<div class="oauth-login" data-username="<?php echo esc_attr($this->envato_username); ?>">
				<a href="<?php echo esc_url($this->get_oauth_login_url(admin_url('admin.php?page=' . envato_market()->get_slug() . '#settings'))); ?>"
				   class="oauth-login-button button button-primary"><?php echo esc_html__('Login with Envato to activate updates', 'somo') ?></a>
			</div>
			<?php
}
